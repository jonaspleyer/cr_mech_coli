...
pub struct RodMechanicsSettings {
    ...
    pub pos: nalgebra::SMatrix<f32, N_ROD_SEGMENTS, 2>,
    /// The current velocity
    pub vel: nalgebra::SMatrix<f32, N_ROD_SEGMENTS, 2>,
...
impl RodMechanicsSettings {
    ...
    fn pos<'a>(&'a self, py: Python<'a>) -> Bound<'a, numpy::PyArray2<f32>> {
        use numpy::ToPyArray;
        let new_array = numpy::nalgebra::SMatrix::<f32, N_ROD_SEGMENTS, 2>::from_iterator(
    ...
    fn set_pos<'a>(&'a mut self, pos: Bound<'a, numpy::PyArray2<f32>>) -> pyo3::PyResult<()> {
        ...
        self.pos = nalgebra::SMatrix::<f32, N_ROD_SEGMENTS, 2>::from_iterator(iter.into_iter());
        Ok(())
    }
    ...
    fn set_vel<'a>(&'a mut self, pos: Bound<'a, numpy::PyArray2<f32>>) -> pyo3::PyResult<()> {
        use numpy::PyArrayMethods;
        let iter: Vec<f32> = pos.to_vec()?;
        self.vel = nalgebra::SMatrix::<f32, N_ROD_SEGMENTS, 2>::from_iterator(iter.into_iter());
        Ok(())
    }
}
...
pub struct AgentSettings {
    mechanics: Py<RodMechanicsSettings>,
    interaction: Py<MorsePotentialF32>,
}
            ...
            agent_settings: AgentSettings {
                ...
                interaction: Py::new(
                    ...
                    MorsePotentialF32 {
                        ...
                        potential_width: 1.0, // MICROMETRE
...
impl RodAgent {
    ...
    fn set_pos<'a>(&'a mut self, pos: Bound<'a, numpy::PyArray2<f32>>) -> pyo3::PyResult<()> {
        ...
        let iter: Vec<f32> = pos.to_vec()?;
        self.mechanics.pos =
            nalgebra::SMatrix::<f32, N_ROD_SEGMENTS, 2>::from_iterator(iter.into_iter());
        Ok(())
    }
...
pub fn run_simulation(
    ...
    std::collections::HashMap<u64, std::collections::HashMap<CellIdentifier, RodAgent>>,
    PyErr,
> {
                    ...
                    pos: nalgebra::SMatrix::<f32, N_ROD_SEGMENTS, 2>::from_fn(|r, c| {
                        p1[0]
                            + r as f32
                                ...
                                * if c == 0 {
                                    (angle * rng.gen_range(0.95..1.05)).cos()
                                } else {
                                    (angle * rng.gen_range(0.95..1.05)).sin()
                                }
                    }),
        ...
        let all_agents = storage_access
            ...
            .map(|(iteration, cells)| {
                (
                    ...
                    cells
                        .into_iter()
                        .map(|(identifier, (cell, _))| (identifier, cell.cell))
