...
def _create_histogram_subplot(
    ...
    ax.plot(bin_centers, hist_real, label='Real',
            color='#2E86AB', linewidth=2, alpha=0.8)
    ax.plot(bin_centers, hist_synth, label='Synthetic',
...
def generate_detailed_plots(
    ...
    try:
        ...
        for real_img_path, mask_path in tqdm(image_pairs, desc="Creating detailed plots"):
            ...
            synth_fg = extract_masked_region(synthetic_img, synthetic_mask, 'foreground')
            synth_bg = extract_masked_region(synthetic_img, synthetic_mask, 'background')
            ...
            _create_histogram_subplot(
                ax_full, real_img, synthetic_img,
                "Full Image",
            ...
            real_display = real_img if len(real_img.shape) == 2 else np.mean(real_img, axis=2)
            synth_display = synthetic_img if len(synthetic_img.shape) == 2 else np.mean(synthetic_img, axis=2)
            ...
            ax_images.text(w//2, h + 15, 'Real', ha='center', fontsize=9, fontweight='bold')
            ax_images.text(w + w//2, h + 15, 'Synthetic', ha='center', fontsize=9, fontweight='bold')

            ax_real_mask = fig.add_subplot(gs[2, 0])
            mask_display = original_mask if len(original_mask.shape) == 2 else np.max(original_mask, axis=2)
            ax_real_mask.imshow(mask_display, cmap='tab20', interpolation='nearest')
            ax_real_mask.set_title("Real Segmentation Mask", fontsize=10, fontweight='bold')
            ax_real_mask.axis('off')

            ax_synth_mask = fig.add_subplot(gs[2, 1])
            synth_mask_display = synthetic_mask if len(synthetic_mask.shape) == 2 else np.max(synthetic_mask, axis=2)
            ax_synth_mask.imshow(synth_mask_display, cmap='tab20', interpolation='nearest')
            ax_synth_mask.set_title("Synthetic Segmentation Mask", fontsize=10, fontweight='bold')
...
def generate_comparison_plot(
    ...
    try:
        ...
        gs = GridSpec(nrows, ncols, figure=fig, height_ratios=[1, 1], hspace=0.3, wspace=0.2)
        ...
        for idx, example in enumerate(examples):
            ...
            hist_real, bins_real = np.histogram(real_display.flatten(), bins=100, range=(0, 1), density=True)
            hist_synth, _ = np.histogram(synth_display.flatten(), bins=100, range=(0, 1), density=True)
            bin_centers = (bins_real[:-1] + bins_real[1:]) / 2

            ax_hist.plot(bin_centers, hist_real, label='Original', color='#2E86AB', linewidth=2, alpha=0.8)
            ax_hist.plot(bin_centers, hist_synth, label='Synthetic', color='#A23B72', linewidth=2, alpha=0.8)
            ...
            h, w = real_display.shape
            ax_img.text(w//2, h + 20, 'Original', ha='center', fontsize=10, fontweight='bold')
            ax_img.text(w + w//2, h + 20, 'Synthetic', ha='center', fontsize=10, fontweight='bold')
