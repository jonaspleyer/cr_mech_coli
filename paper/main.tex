...
\subsubsection{Imaging-Space Metric}
...
However, the exact time point for the fission event can not be determined accurately in practice,
...
Such uncertainties which are also common problems for segmentation methods, further highlight the
importance of a purely imaging-based comparison scheme.

These thoughts explain why the direct metric can not be simply reused on confined intervals which
...
This fundamental limitation will be circumvented by the imaging-based method which we propose.

\paragraph{The Metric}
We develop an approach which can be understood as a metric in image space where we quantify
differences in the visual footprint of two bacterial configurations.
...
% To compare the differing cellular configurations, we require additional
% methods which can be used in time intervals containing cellular divison events.
We can use the previously introduced \nameref{subsection:visualization} methods to obtain cell-masks
...
Whenever two colors are not matching, a penalty is assigned for each point in the pixel-grid.
The total cost is the sum of all penalties.
With this approach it is important to take into account the parental relationship between the cells,
...
We assign a specific parent penalty $0\leq p\leq 1$ value for these points which should be seen as
...
This means that other models who even utilize very distinct cellular representations but are able to
render segmented images can reuse our method.

\subsubsection{Testing the Comparison Algorithms with Division Events}
When utilizing a computational model (see \ref{sec:computational-model}), we can generate
