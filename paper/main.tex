...
% \textbf{Restructure Introduction with these bullet points}
% \begin{itemize}
%     \item Problem 1: Parameter estimation of \acp{abm}: how to do at all?
%     \item Problem 2: Many \acp{abm} have very many parameters
%     \item Problem 3: No \ac{abm} framework with support for flexible rod-shaped bacteria
%     \item Literature Gap 1: noone really does parameter estimation for \acp{abm}
%     \item Literature Gap 2: also no methods do this
%     \item Literture  Gap 3: Some literature on flexible rods and some models but often not with
%         growth, reactions etc.
%     \item Why matter 1: No explainability without values of parameters.
...
%     \item What we do 2: Develop methods to compare numerical results with data (for previous model)
%     \item What we do 3: Apply these methods in 3 scenarios covering different aspects
%     \item Why my contribution significant 1: Show that parameter estimation can be done
%     \item Why my contribution significant 2: Show how to treat heterogeneity and that we can map 1:1
%         of model to cells
% \end{itemize}

\acp{abm} are powerful toosl to study complex cellular systems~\cite{Pleyer2023}.
...
reality.
Their applications range from cancer research~\cite{Ghaffarizadeh2018,Cooper2020} over
Immuno-Oncology~\cite{Karolak2021} to the modeling of mechanical intercellular connections in plant
cells~\cite{DeRybel2014} which demonstrates their ability to capture complex emergent multicellular
behavior.
However, despite their popularity \acp{abm} face multiple practical challenges.

One major problem is how to effectively perform parameter estimations for \acp{abm}.
...
Many \acp{abm} contain a substantial amount of parameters which poses another major problem
related to the tuning of the model and can limit it's biology interpretability.
Therefore, many studies only show that a suitably chosen set of parameters produces the desired
biological behavior and asses how a subset of parameters can effect their results.
...
Is there a more simple, i.e. reduced model that can describe the same phenomena?
If \acp{abm} are not able to answer these questions, their results will significantly lack in
explanatory power and ability to link to the biological problem at hand.
In addition, biolgical \ac{abm} are typically designed with specific systems in mind and come with a
predefined set of parameters which need to be specified and are not easily adaptable.
This circumstance leads to problems when aiming to find a minimal set of identifiable parameters
that can describe the desired phenomena.

Within our approach we will utilize a new computational model of flexible, elongated bacteria which
...
but their model can not be transferred directly to more complex biolgical systems which might
involve intracellular reactions, extracellular processes, growth and division.
This poses another problem in the computational study of these systems.

The aforementioned problems present scientific significant gaps.
...
Furthermore, models which are not properly calibrated can not guarantee to accurately represent the
underlying biolgical reality beyond numerical coincidence.
Finally, elongated bacteria are fundamental biolgical organisms which can be seen as a precursor to
...
emergent phenomana such as the formation of biofilms~\cite{Li2025} or bacterial
...
compare numerical results with biolgical data on a single-cell level.
With these methods we can estimate the parameters of our model in three scenarios, each probing
different aspects of our model with increasing level of complexity.

Our approach demonstrates that parameter estimation of \acp{abm} is indeed feasible, even for
complex models and when incorporating cellular heterogeneity.
...
\textbf{TODO Talk about how to fit individual bacteria and why it is important.}
...
\textbf{Put this in Supplement and discuss it further. Still link to it here.}\\
