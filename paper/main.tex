...
\begin{equation}
    \vec{F}_{i,\text{external}} = C |\vec{x}_i - \vec{x}_{i+1}|(y-y_b)\sin(\beta_i)\vec{e}_x
\end{equation}
which is proportional to the length of the rod segment projected along the y-axis.
It acts similarly to Stoke's Law~\cite{Stokes1856} along the axis of the fluid flow (x-axis).
Here $\beta_i$ is the angle of the rod edge between $\vec{x}_{i+1}-\vec{x}_i$ and the x-axis.
Amir et. al~\cite{Amir2014_2} calculated the flow profile for this particular setup.
...
We assume that these time intervals are identically long since the data from the movies does not
allow for a finer resolution of the dynamics.

Since this model is void of any cell-cell interactions or cell division, the dynamics of the cell
...
interval.
\textbf{Plots are not in supplement anymore}
The corresponding plots can be seen in \textbf{TODO}.
