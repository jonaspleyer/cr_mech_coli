...
\begin{equation}
    \vec{F}_{i,\text{external}} = \frac{C}{2} |\vec{x}_i - \vec{x}_{i+1}|(yR - y^2)\sin(\beta_i)\vec{e}_x
\end{equation}
...
The flow profile for this particular setup is proportional to $(2yR-y^2)$ where $y$ is the vertical
displacement measured from the end of the tube~\cite{Wolschin2021}
(see Figure~\ref{fig:amir-bending-simulation}) and $R$ is the total size of the chamber along the
y-axis.
Since the bacterium does not reach the peak of the velocity profile, we can estimate the slope via a
linear dependence on the pertruding segment along the y-axis~\cite{Wolschin2021}.
