...
\title{Parameter Estimation Methods for Models of Rod-Shaped Bacteria}
...
\renewcommand{\shorttitle}{Parameter Estimation Methods for Models of Rod-Shaped Bacteria}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \node (model) [rect, align=center] at (0,0) {Mathematical \&\\ Computational Model};
        \node (extraction) [rect] at (6, 0) {Data Extraction};
        \node (params) [rect] at (6, -2) {Parameter Estimation};
        \node (visualization) [rect] at (0, -2) {Visualization};

        \draw[arrow] (model.east) -- (extraction.west);
        \draw[darrow] (params.north) -- node[anchor=south, rotate=-10] {validate} (model.south);
        \draw[arrow] (model.south) -- (visualization.north);
        \draw[arrow] (visualization.east) -- (params.west);
        \draw[arrow] (extraction.south) -- (params.north);

        % Future stuff
        \node (realimages) [frect] at (0, -4) {Data Generation};
        \node (segmentation) [frect] at (6, -3.5) {Cell Segmentation};
        \node (tracking) [frect] at (6, -4.5) {Cell Tracking};

        \draw[arrow] (visualization.south) -- (realimages.north);
        \draw[arrow] (realimages) -- (segmentation.west);
        \draw[arrow] (realimages) -- (tracking.west);
    \end{tikzpicture}
    \caption{
