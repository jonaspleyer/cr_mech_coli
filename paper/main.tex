...
comparison algorithm (see section~\ref{subsec:image-space-metric}).
...
metric (see section~\ref{subsec:image-space-metric}) and for testing purposes.
...
\subsubsection{Direct Skeleton-Space Metric}
\label{subsec:skeleton-metric}
...
matter of comparing two of these arrays via a suitable cost function~\cite{Wang2020}.
...
To explain why the direct skeleton-space metric can not be reused on confined
intervals which do not include division events and for further details, we refer the interested
reader to the supplementary material~\ref{sec:supplement-comparison-algorithms}.
...
usage of the image-space metric.
...
ones using the direct comparison algorithm (see Subsection~\ref{subsec:skeleton-metric}).
...
section~\ref{subsec:skeleton-metric}) while the latter approach has to account for division events
and thus requires the image-space metric method (see section~\ref{subsec:image-space-metric}).
...
\ref{subsec:skeleton-metric}) for our cost function.
...
cell division events which means that we now have to use the image-space metric (see
...
The image-space metric is computationally much more expensive compared to the direct method.
...
utilized as additional feedback source.
Every text was written and audited by the authors which is why they take full responsibility for the
contents presented in this publication.
...
%###################################################################################################
\subsection{Details for the Image-Space Metric}
