...
\subsubsection{Lineage-aware Imaging-Space Metric}
...
To capture cell-cell heterogeneity, the parameter estimation framework needs to support both global
(population-wide) and local (cell-specific) parameters.
In practice it is often desirable to utilize a mixed approach in which parameters corresponding to
...
We identify a subset $N_g\leq N_p$ of global parameters for shared behavior and $N_l=N_p-N_g$ local
...
interval.
This value includes cells which have divided or died during the inspected time interval or may only
be present for a subset of the total time span.

The volume of the search space scales exponentially with $N_\text{tot}$ and therefore also with the
number of local parameters $N_l$ or number of cells $N_c$ respectively.
Keeping the number of individually assigned parameters $N_l=N_p-N_g$ minimal is crucial to retain
...
Our developed methods are independent of these choices, allowing for flexible assignment strategies.
