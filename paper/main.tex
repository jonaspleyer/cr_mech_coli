...
We use the notation $\vec{w}i:=\vec{x}_i-\vec{x}_{i-1}$ with which the resulting force acting
between two vertices if given by

\begin{equation}
    \vec{F}_{i,\text{springs}} =
        -\gamma\left(1 - \frac{l}{\left|\vec{w}i\right|}\right) \vec{w}i
        + \gamma\left(1 - \frac{l}{\left|\vec{w}{i+1}\right|}\right) \vec{w}{i+1}.
\end{equation}
...
Given the angle $\alpha_i = \sphericalangle(\vec{w}{i-1},\vec{w}i)$ between adjacent edges,
...
\begin{equation}
    \vec{F}_{i,\text{curvature}} = \eta\kappa_i
        \frac{\vec{w}i - \vec{w}{i+1}}{|\vec{w}i-\vec{w}{i+1}|}
\end{equation}
