...
\begin{figure}[H]
    ...
    \caption{
        ...
        The individual components rely on a unified spatial representation (left) which discretizes
        bacteria into vertices (see \nameref{subsec:spatial-representation}).
        Building on top are visualization methods (top), a novel computational model for flexible
        elongated bacteria (middle) and data extraction routines (botom).
        ...
        These components can be combined to estimate the parameters of the developed computational
        model which can then be used in generating synthetic data (top, right).
        Colors indicate the componet type; core scientific drivers (red), auxiliary methods (yellow)
        and extensions (blue).
    }
