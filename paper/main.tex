...
We set out to estimate the parameters of every configuration.

First, we fit our model to data of elastically bending bacteria confined in a microfluidic
apparatus which was previously published by Amir et. al~\cite{Amir2014,Amir2014_2}.
This scenario isolates the mechanics of the flexible rod and is void of any complex cell-cell
interactions.
...
(see subsection~\ref{section:application-2}).
Initially, we consider a time interval that does not contain division events and are thus able to
employ the direct metric (see \nameref{subsec:direct-metric}).
Finally, we apply the image-space metric (see \nameref{subsec:image-space-metric}) to a time-series
containing multiple division events, demonstrating the feasibility of our approach in such
scenarios.
