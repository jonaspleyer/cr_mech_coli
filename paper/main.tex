...
\begin{figure}[H]
    ...
    \end{tikzonimage}%
    \begin{tikzonimage}[width=0.5\textwidth]
        {docs/source/_static/morse-potential-shapes.pdf}
...
However, Figure~\ref{fig:amir-bending-simulation} (E-G) exhibit multimodal minima which can not be
distinguished.
...
As we have explained earlier (Section~\ref{sec:computational-model}), $\gamma$ can be viewed as an
auxiliary parameter.
...
the non-identifiability of the damping constant $\lambda$, since this force takes no effect in the
...
\ac{ecoli}.
We consider two scenarios to apply the previously developed metrics within a realistic context and
...
Figure~\ref{fig:application-2-data-overview} (D) shows the length of the rods as calculated via our
extraction algorithm.
To quantify this heterogeneity, we fit the individual curves with an exponential function
\begin{equation}
    l(t) = l_0 \exp(\lambda t).
\end{equation}
where $l_0$ is the starting length of the rod and $\lambda$ is the growth rate.
Figure~\ref{fig:application-2-data-overview} (E) shows the distribution of the obtained growth rates.
...
We first restrict our analysis to a time interval prior to the first division event which allows us
to use the computationally more efficient direct position-space metric
(Section~\ref{subsec:image-space-metric}).

Utilizing our full mechanical model (Section~\ref{sec:computational-model}), we optimized a total
...
algorithm~\cite{Storn1997} which was initially seeded with Latin-Hypercube sampling~\cite{McKay1979}.
...
Subplot (A) displays the profile-likelihood for the damping constant $\lambda$ and exhibits a wide
...
While the potential stiffness (B) and radius (F) can be estimated well with the given data, the
potential strength (E) contains a practical unidentifiability~\cite{Raue2009} which is lifted when
fixing the damping constant $\lambda=\SI{1}{\per\minute}$.
In contrast to the Morse potential, the Mie potential uses two parameters $n,m$ (C,D) to determine
the shape of the interaction potential which allows it to describe a larger class of curves.
However, as subplots (A,C,D,E) show, neither the full Mie-type interaction nor a reduced model with
$\lambda=\SI{1}{\per\minute}$ allow for the estimation of the exponents $n,m$.
Although a minimum could be determined and the steepness of the enclosing profile towards the larger
values of exponents $n,m$ increases for the reduced model, the confidence intervals clearly show a
practical non-identifiability.
Since the reduction of the model improves the situation we are hopeful that an increased data set
...
The growth rates (G-L) can all be estimated for any interaction type but the non-reduced Mie-type
interaction shows wider confidence intervals, thus greater uncertainty in the predicted parameter
value.

\begin{figure}[H]
    \centering
    \begin{tikzonimage}[width=0.49\textwidth]
        {figures/crm_fit/morse_partial/snapshots/predicted-006667.png}%
        \node at (0.025, 0.975)[anchor=north west, white]{\textbf{A}};
    \end{tikzonimage}%
    \hspace{0.01\textwidth}%
    \begin{tikzonimage}[width=0.49\textwidth]
...
\begin{figure}[H]
    ...
    \end{tikzonimage}%
    \caption{
        (A-L) Likelihood profiles for 4 scenarios: Full model with Morse/Mie interaction and with a
...
In addition to the previously used parameters, we also need to consider division lengths at
which the bacteria split and the new individual growth rates of the daughter cells as additional
parameters.
Since our simulation involves the division of 4 cells into 8 new ones, this amounts to a total of $8$
new growth rates and $4$ division lengths.
...
Image-Space metric for maximum-likelihood approaches, we simply display the change of the cost
function when fixing a single parameter.
...
Figure~\ref{fig:likelihood-profiles-comparison-with-division} shows that some curves (F,G,I,L)
exhibit a plateau close to the calculated optimum of the estimated parameter.
...
This presents a restriction on the lower and upper bounds of the parameter value.
However for some parameters (H,J,K) the lower region of the plots contains a slight gradient,
indicating that our mechanistic model is able to provide some level of information about the missing
datapoints.
...
Rendering of the masks dominates the workload ($69.2(1.4)\%$), followed by the model prediction
($15.63(21)\%$) and mask adjustment ($14.49(19)\%$) while the actual pixel-wise compariosn is
negligible ($0.68(5)\%$) (see also Section \ref{section:supplement-performance}).
The mask-adjustment step is necessary for every individual numerical simulation to ensure that masks
obtained from the data are color-comparable to the ones produced by our computational model.
To this end, we need to construct a set of family trees (forest) of the data-bacteria and compare
with the \textit{in-silico} family trees.
...
\begin{figure}[H]
    ...
    \caption{
        ...
        division lengths.
        \textbf{TODO add profiles for daughter cells}
        \textbf{TODO possibly extend this description?}
    }
    \label{fig:likelihood-profiles-comparison-with-division}
\end{figure}

\textbf{TODO write text to compare the numerical values. This needs updated input from results.}
Table~\ref{tab:comparison-growth-rates} shows the obtained values for the growth rates via the
different techniques.
Although numerical values differ between the individual settings, each estimation technique was able
to capture the same trends.
Cell $0,1,3$ tend to be the fastest growing cells while $2,4,5$ are slower.
Furthermore, we can see that daughter cells tend to grow quicker compared to mother cells,
indicating the their parents were still adapting to the new environment.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c c c c}
        % \multicolumn{7}{l}{}\\
        & \multicolumn{6}{c}{Growth Rates $[\mu]=\SI{}{\per\minute}$}\\
        ...
        % 0.00671 & 0.00746 & 0.01138 & 0.00848 & 0.02005 & 0.00952\\
        Morse
            & 0.0070
            & 0.0078
            & 0.0118
        ...
        \multicolumn{7}{l}{\textbf{Lineage-Aware Image-Space Metric (TODO)}}\\
        \midrule
        Imaging \ac{abm} Mie & 0.00104 & 0.00146 & 0.00206 & 0.00286 & 0.00190 & 0.00171\\
        % & \textbf{Daughters}\\
        % \midrule
        % & Cell $0$ & Cell $1$ & Cell $2$ & Cell $3$ & Cell $4$ & Cell $5$\\
        Daughter 0 & 0.00240 & 0.00494 & 0.00096 & 0.00363 & - & -\\
        Daughter 1 & 0.00490 & 0.00226 & 0.00448 & 0.00048 & - & -\\
    ...
    \end{tabular}
    \caption{
        \textbf{TODO maybe report uncertainties?;
        Update and explain these values. And also add units!;
...
%###################################################################################################
\paragraph*{Application Data \& Preparation}
\label{sec:supplement-data-details}

\subparagraph*{Application 1}

\subparagraph*{Application 2}
\begin{itemize}
    \item Data from IWF Goettingen \cite{Trude1982}
    \item $\SI{15}{\pixel\per\micro\metre}$
    \item $8$ images are equivalent to $\SI{20}{\minute}$
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/crm_fit/optimization-progression.pdf}
    \caption{TODO}
\end{figure}
...
\begin{figure}[H]
    ...
    %     {figures/crm_fit/morse_profiles/profile-growth-rate-0.pdf}%
    % \includegraphics[width=0.33\textwidth]
    %     {figures/crm_fit/morse_profiles/profile-growth-rate-1.pdf}%
    % \includegraphics[width=0.33\textwidth]
    %     {figures/crm_fit/morse_profiles/profile-growth-rate-2.pdf}\\%
    % \includegraphics[width=0.33\textwidth]
    %     {figures/crm_fit/morse_profiles/profile-growth-rate-3.pdf}%
    % \includegraphics[width=0.33\textwidth]
    %     {figures/crm_fit/morse_profiles/profile-growth-rate-4.pdf}%
    % \includegraphics[width=0.33\textwidth]
    %     {figures/crm_fit/morse_profiles/profile-growth-rate-5.pdf}
