...
different aspects of our model with increasing levels of complexity. A key feature of this framework is its modularity.
...
\begin{figure}[H]
    ...
    \caption{
        ...
        The building blocks are visualisation methods (top), a novel computational model for
...
adherance~\cite{Verwey1947,Trejo2013} {\bf [what about repulsion?]}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{c l X}
        ...
        (2) & Growth &
            Cells grow exponentially by inserting new material either along the cylindrical part of the
            rod or at the tip {\bf [das führt doch zu linearem Wachstum?]}~\cite{Robert2014,Takeuchi2005}.\\
        (3) & Division &
        ...
        (4) & Adhesion &
            ...
            distances {\bf [keine Repulsion nötig?]}~\cite{Verwey1947,Trejo2013}.\\
        \bottomrule
    \end{tabularx}
    \caption{
        ...
        assumptions can also be loosened if required {\bf [unverständlich, klingt wie ein Widerspruch]}.
    }
...
This assumption {\bf [wo ist die Assumption?]} alone is enough to be able to build visualization
...
{\bf [natürlich, in einer freien Wissenschaft sollte es so sein. Was meinst du hier genau?]}.
...
\begin{figure}[H]
    ...
    \caption{
        ...
        Vertices on the upper agent are interacting with nearest points on the polygon of the lower
        agent.
...
regions of negative curvature thus introducing heterogeneous cell growth~\cite{Ursell2014}
{\bf [das "thus" legt nahe, dass die Verbindung zwischen "negative curvature"
und "heterogeneous growth" klar ist. Mir ist das überhaupt nicht deutlich]} which
...
external environment~\cite{Wang2010} {\bf [Unklar]}. We model the bacterium as a bending beam and
...
With the notation $\vec{w}_i:=\vec{x}_i-\vec{x}_{i-1}$ the resulting force acting between two vertices is given by:
\begin{equation}
...
paths of the cells. To reduce the computational costs we use an approximative, simplified model.
...
for some specific $j$. This approximation is especially justified for short-ranged interactions.
...
{\bf [das stimmt nicht für $r\in\mathbb{R}$]} which makes it particularly
...
\end{equation}
and has 3 parameters. The overall potential strength $V_0$, the radii of the particles $R_1,R_2$ and the potential
stiffness $\omega$ which controls the rate of change as one travels along the radial axis.
 {\bf [wäre es nicht besser $V_0$ vom Potential abzuziehen, dann wäre $-V_0$ das Minimum und $\lim_{r\to\infty}V(r)=0$?]}
...
To simulate cellular growth, we introduce an exponential growth term for the spring lengths $l$ \cite{Takeuchi2005}.
\begin{equation}
    \partial_t l = \mu l
\end{equation}
...
To construct a new cell, we cannot simply copy the existing one twice,
{\bf [die Aufteilung der Moleküle nach dem Teilungsprozess ist Binomialverteilt, darauf sollte hier eingegangen werden]}
but we also need to adjust internal parameters in the process.
The following actions need to be taken for the old and new agent
{\bf [wie passiert die Aufteilung genau? Werden zwei neue Agenten erzeugt? Ich dachte das sei so.]}.
...
This relation ensures that that the tips of the two new rods are touching right after the division event.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c l c c}
        ...
        Strength {\bf [ich würde es nicht Strength nennen, ist ein unklarer Begriff]} & $V_0$ & $\unit{\micro\metre^2\per\minute^2}$\\
        Exponent 1 & $n$ & $1$\\
        Exponent 2 & $m$ & $1$\\
...
computational model. It is split into 3 parts, each covering distinct purposes.
{\bf [unklar worauf sich "it" bezieht. Das Kapitel hat 4 Abschnitte]}
...
Our visualisation routine relies on a spatial representation in the form of vertices, as well as on
...
The first stage of the visualisation process involves generating three-dimensional meshes for each
bacterium as an intermediate representation (see Figure~\ref{fig:progression-image-generation}).
...
{\bf [wie soll man sich das mit den überlappenden Netzen vorstellen?]}
A render including smoothened meshes and external light sources of this configuration can be seen
in Figure~\ref{fig:progression-image-generation} (B).
...
will use identical colors and colors are unique across the duration of the simulation.
...
{\bf [Das ist leider ziemlich unklar. Du musst erklären, was den das Problem ist mit den "higher and lower values" und den "masks" ist.]}.
...
images~\cite{Stringer2020,Cutler2022,Hardo2022}. Figure~\ref{fig:position-extraction-algorithm} (A) shows a typical microscopic image.
Our algorithm requires that images are prepared using a pixel-based (in-image) encoding scheme, as we explained in section~\ref{subsection:visualization}.
...
The skeletonisation algorithm provides us with a set of unordered lattice points $\vec{p}_i$
(pixel coordinates) that define the location of the skeleton. 
...
If we encounter fewer than two or more than two points that fit this criterion, the routine cannot
commence and is aborted.

Next, we start with one of the endpoints and determine the next neighbor and continue in this way
until we reach the second endpoint, thus forming a sorted polygon of pixel coordinates.
...
{\bf [Das ist unklar. Wie viele $p_i$ gibt es?]}
...
which can be minimized. As explained in Section~\ref{sec:methodology}, parameters can be specified
as global, which are identical across all cells, or as individual, which allow us to capture
cell-to-cell variability.
We use the preceding algorithms to extract data from cell-masks, simulate from an initial state
{\bf [wie ist der Anfangszustand gewählt?]}
...
{\bf [Das ist verwirrend. Es ist nicht nötig zuerst die LS einzuführen und dann die Likelihood. Starte mit der Likelihood, oder dem Posterior. Die Profile-Likelihood ist dann eine Möglichkeit die Konfidenzen zu bestimmen.]}
...
{\bf [wir müssen über diesen Abschnitt reden; deine Argumentation scheint mir etwas zu umständlich und "hintenrum"]}
...
division event occurs since this would require comparing sets of unequal cardinality.
...
bacteria at the next time step. Such a mismatch can happen, e.g. due to non-matching division times of model and data.
...
{\bf [das verstehe ich nicht, man kann doch Simulation und Daten nur an festen, diskreten Zeitpunkten vergleichen und an denen sollte die Anzahl der Zellen doch gleich sein.]}
...
However, all of these statistical methods are based on the idea of identifying matching and non-matching regions. Therefore, it is first necessary to define a criterion for this.
...
{\bf [Das ist unklar. Woher kommt denn in 2D ein Überlapp?]}

These overlaps stem from parameter combinations which exhibit a low repulsive interaction force.
Since such overlaps are unrealistic, we need to assign an additional penalty $p_o\geq0$ in this
case.
...
other cell.
{\bf [Ich denke nicht, dass jemand versteht worüber du hier redest.]}

Figure~\ref{fig:mask-difference-metric} (A,B) shows two states of a numerical simulation
with varying numbers of cells. We can see that some cells have divided while others have only
extended in the same time interval.
...
increasing in complexity. We estimate the parameters for every configuration.
{\bf [das Wort configuration ist hier unklar]}
