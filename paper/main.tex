...
generalizability \textbf{TODO CITATION}. Typically researchers estimate the parameter of their model
using methods such as Maximum Likelihood or Maximum a Posteriori.

[Gabor, Attila, and Julio R. Banga. ‘Robust and Efficient Parameter Estimation in Dynamic Models of Biological Systems’. BMC Systems Biology 9, no. 1 (2015): 74. https://doi.org/10.1186/s12918-015-0219-2., 
Banga, Julio R, and Eva Balsa-Canto. ‘Parameter Estimation and Optimal Experimental Design’. Essays in Biochemistry 45 (2008): 195–209. https://doi.org/10.1042/BSE0450195., 
Ashyraliyev, Maksat, Yves Fomekong-Nanfack, Jaap A Kaandorp, and Joke G Blom. ‘Systems Biology: Parameter Estimation for Biochemical Models’. FEBS Journal 276, no. 4 (2009): 886–902. https://doi.org/10.1111/j.1742-4658.2008.06844.x.] \\

check for parameter identifiability using methods like profile-likelihood or apply model reduction techniques \\

[Kreutz, Clemens, Andreas Raue, Daniel Kaschek, and Jens Timmer. ‘Profile Likelihood in Systems Biology’. The FEBS Journal, May 2013, n/a-n/a. https://doi.org/10.1111/febs.12276.
Raue, Andreas, Johan Karlsson, Maria Pia Saccomani, Mats Jirstrand, and Jens Timmer. ‘Comparison of Approaches for Parameter Identifiability Analysis of Biological Systems.’ Bioinformatics (Oxford, England), ahead of print, January 2014. https://doi.org/10.1093/bioinformatics/btu006.
Simpson, Matthew J. ‘Data-Informed Model Reduction for Inference and Prediction from Non-Identifiable Models’. Preprint, Synthetic Biology, 21 March 2025. https://doi.org/10.1101/2025.03.20.644462.]\\
...
\textbf{[Was mir hier nicht gefällt, ist, dass du von Parameter schätzen bei ABMs kommend ziemlich detailliert über Mechanik redest. Das passt nicht. Man erwartet mehr Infos über den Blueprint und ist verwirrt. Worum geht es hier?]}
...
\textbf{[Ich denke nicht, dass $\vec{x}_i$ hier nötig ist. Es bietet keinerlei Information. Du kannst alles so schreiben ohne das mathematische Symbol. Du musst den Bogen hinbekommen von dem allgemeinen Problem des Parameterschätzen bei ABMs zu dem spezifischen Problem, welches du hier verhandelst.]}
...
We will go into the details of the model for rod-shaped bacteria.
...
We are thus aiming to work with a minimal set of assumptions that is as general as possible but
still specific to rod-shaped bacteria. 

Given this delicate balance, it is evident that numerous cellular processes will be excluded due to
their specificity to particular cell types or their irrelevance to the construction of our model.
This encompasses a range of effects, including polar interactions~\cite{Duvernoy2018}, extracellular
reactions~\cite{Wang2010} or differentiation processes~\cite{vanGestel2015,Lpez2010}, among numerous
others.
When describing the bacterial system, we distinguish between intra-cellular processes and cell-cell
interactions.
We restrict ourselves to the features discussed below and summarized in
...
by~\cite{Ursell2014}. They found that MreB localizes specifically at regions of negative curvature
and thus introduces
...
external environment. These results were also confirmed by the studies of~\cite{Wang2010}.
In their experiments, a single bacterium was fixed at the bottom pole while an optically trapped
bead was placed on the upper pole. They could now exert a force on the free part and thus study the
response of the cell under different conditions. When treating cells with A22, an antibiotic which
disassembles MreB~\cite{IWAI2002,Gitai2005,Karczmarek2007,Bean2009}, a higher displacement with
identical force could be measured, thus indicating that MreB increases mechanical rigidity of the
cell.
...
\textbf{[Bitte etwas kürzen, da du hier keinen Review schreibst und dich deswegen um Prägnanz bemühen musst.]}
...
\textbf{[Man versteht nicht so recht, warum du das alles hier erzählst und was daraus folgt. Die Verbindung zu deinem Modell fehlt.]} 
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{c l X}
        ...
        (4) & Process Parameters &
            We take a potential heterogeneity of the bacteria into account by \textbf{[wir sollten über diesen Punkt nochmals reden. 
            Das ist hier leicht missverständlich. Parameter sind auch kein intrazellularer Aspekt.]} \cite{Koutsoumanis2013}.\\
...
collection of auxiliary vertices $\{\vec{x}_i\}$ which are connected by springs in ascending order.
Furthermore, we assume that the cells are flexible described by their rigidity. A force $\vec{F}$
interacting between cellular agents determines the radius (thickness) of the rods and an attractive
component can model adhesion between cells.
