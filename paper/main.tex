...
form of collections of vertices from segmented microscopic images.
...
We will test and benchmark the constructed algorithms within the coming sections.

It should be noted that these methods only depend on the spatial representation described in
section~\ref{subsec:spatial-representation}.
...
Furthermore, the developed methods are independent of how we assign parameters within our model.
This means we can opt to incorporate intracellular variability by specifying parameters on a
per-cell basis.
Or we can employ shared parameters between all agents for processes which can be expected to behave
identical within margin of error.
Furthermore, mixed approaches where some parameters are assigned globally and others individually
...
For the particular model chosen here, a list of parameters can be seen in
table~\ref{table:list-of-parameters} but the following calculations are equally true for any
single-cell model with $N_p$ number of parameters.
Consider the case where we identify a subset $N_g\leq N_p$ of parameters which will be treated
globally, thus with equal numerical value for every cell.
This means that $N_p - N_g$ parameters will be assigned individually for each of $N_c$ number of
cells.
...
scenarios where different strategies of parameters assignment are desired, e.g. when considering
two distinct species of cells which should be described by 2 parameters in total regardless of the
number of cells.
